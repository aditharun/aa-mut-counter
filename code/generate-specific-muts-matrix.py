...
parser.add_argument('--mutations_file', type=str, help='text file with list of mutations to keep each on a new line w/o a comma separation') 

args = parser.parse_args()

filepath_mutations = args.mutations_file

specificmutations = open(filepath_mutations, "r") 
specificmutations_data = specificmutations.read() 
muts_to_keep = specificmutations_data.split("\n")
print(muts_to_keep)
specificmutations.close()

mutbasename = os.path.basename(filepath_mutations)
...
outpath = os.path.join(cwd_results, "counts-" + mutbasename)
...
df_mutfiles_aa=df_mutfiles[df_mutfiles["HGVSp"].isin(muts_to_keep)].reset_index()
...
print('Done. Results stored in results/ folder for ' + mutbasename)
